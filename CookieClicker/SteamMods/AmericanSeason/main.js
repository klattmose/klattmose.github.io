...
AmericanSeason.GameVersion = '2.053';

AmericanSeason.launch = function(){
	AmericanSeason.init = function(){
		AmericanSeason.modDir = CCSE.GetModPath(AmericanSeason.name);
		AmericanSeason.iconsURL = AmericanSeason.modDir + '/customIcons.png';
	...
	AmericanSeason.getMenuString = function(){
		...
		function inputBoxListing(prefName, prefDisplayName, desc){
			...
			listing += m.InputBox('AS_' + prefName, 65, AmericanSeason.config[prefName], "AmericanSeason.UpdatePref('" + prefName + "', this.value)")
			listing += '<label>' + prefDisplayName + (desc ? ' : ' + desc : '') + '</label>';
			listing += '</div>';
			return listing;
		}
		
		var str = '<div class="listing">' + m.ActionButton("AmericanSeason.config = AmericanSeason.defaultConfig(); Game.UpdateMenu();", 'Restore Default') + '</div>';
		str += '<div class="listing">' + m.ToggleButton(AmericanSeason.config, 'SHOW_CANVAS', 'SHOW_CANVASButton', 'Canvas ON', 'Canvas OFF', 'AmericanSeason.Toggle') + '<label>Display fireworks in the left panel. Inhibits clicking wrinklers.</label></div>';
		
		str += m.Header('Projectiles');
	...
	AmericanSeason.createShimmer = function(){
		...
		var shimmer = {
			...
			initFunc: function(me){
				...
				me.l.style.backgroundImage = 'url("' + AmericanSeason.modDir + '/rocket.png")';
		...
		Sounds['snd/rocketLaunch.mp3'] = new Audio(AmericanSeason.modDir + '/rocketLaunch.mp3');
		Sounds['snd/rocketBoom.mp3'] = new Audio(AmericanSeason.modDir + '/rocketBoom.mp3');
	}
